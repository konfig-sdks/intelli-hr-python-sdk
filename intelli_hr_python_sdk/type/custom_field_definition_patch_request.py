# coding: utf-8

"""
    intelliHR Public API

    You can find developer's guide and more documentation on [https://developers.intellihr.io](https://developers.intellihr.io)

    The version of the OpenAPI document: V1
    Contact: support@intellihr.co
    Generated by: https://konfigthis.com
"""

from datetime import datetime, date
import typing
from enum import Enum
from typing_extensions import TypedDict, Literal, TYPE_CHECKING

from intelli_hr_python_sdk.type.custom_field_definition_patch_request_select_definition import CustomFieldDefinitionPatchRequestSelectDefinition

class RequiredCustomFieldDefinitionPatchRequest(TypedDict):
    pass

class OptionalCustomFieldDefinitionPatchRequest(TypedDict, total=False):
    # The description of this [Custom Field Definition](https://developers.intellihr.io/docs/v1/). This is used as a tooltip on the create and update pages.
    description: typing.Union[str, none_type]

    # Name given to this [Custom Field Definition](https://developers.intellihr.io/docs/v1/). This name would normally be shown to users of the system.
    name: str

    # Whether or not this [Custom Field Definition](https://developers.intellihr.io/docs/v1/) is enabled to be used.
    isEnabled: bool

    # Whether or not this [Custom Field Definition](https://developers.intellihr.io/docs/v1/) is marked as sensitive information.
    isSensitive: bool

    selectDefinition: CustomFieldDefinitionPatchRequestSelectDefinition

    # This field is used in conjunction with type `PEOPLE_DROPDOWN`
    peopleDropdownDefinition: typing.Union[bool, date, datetime, dict, float, int, list, str, None]

class CustomFieldDefinitionPatchRequest(RequiredCustomFieldDefinitionPatchRequest, OptionalCustomFieldDefinitionPatchRequest):
    pass
