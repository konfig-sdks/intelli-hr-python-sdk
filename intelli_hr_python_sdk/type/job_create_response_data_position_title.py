# coding: utf-8

"""
    intelliHR Public API

    You can find developer's guide and more documentation on [https://developers.intellihr.io](https://developers.intellihr.io)

    The version of the OpenAPI document: V1
    Contact: support@intellihr.co
    Generated by: https://konfigthis.com
"""

from datetime import datetime, date
import typing
from enum import Enum
from typing_extensions import TypedDict, Literal, TYPE_CHECKING


class RequiredJobCreateResponseDataPositionTitle(TypedDict):
    pass

class OptionalJobCreateResponseDataPositionTitle(TypedDict, total=False):
    # The identifier string for the [Position Title](https://developers.intellihr.io/docs/v1/) of this [Job](https://developers.intellihr.io/docs/v1/).
    id: str

    # A name given to this [Position Title](https://developers.intellihr.io/docs/v1/)
    name: str

    # An optional code that relates to this [Position Title](https://developers.intellihr.io/docs/v1/)
    code: typing.Union[bool, date, datetime, dict, float, int, list, str, None]

class JobCreateResponseDataPositionTitle(RequiredJobCreateResponseDataPositionTitle, OptionalJobCreateResponseDataPositionTitle):
    pass
