# coding: utf-8

"""
    intelliHR Public API

    You can find developer's guide and more documentation on [https://developers.intellihr.io](https://developers.intellihr.io)

    The version of the OpenAPI document: V1
    Contact: support@intellihr.co
    Generated by: https://konfigthis.com
"""

from datetime import datetime, date
import typing
from enum import Enum
from typing_extensions import TypedDict, Literal, TYPE_CHECKING

from intelli_hr_python_sdk.type.custom_fields_response import CustomFieldsResponse
from intelli_hr_python_sdk.type.locations_data_representatives import LocationsDataRepresentatives

class RequiredLocationsData(TypedDict):
    pass

class OptionalLocationsData(TypedDict, total=False):
    # The identifier string for the [Location](https://developers.intellihr.io/docs/v1/).
    id: str

    # The identifier string for the parent [Location](https://developers.intellihr.io/docs/v1/), or null if there is no parent (this is a top level location).
    parentId: typing.Union[str, none_type]

    # Name given to this [Location](https://developers.intellihr.io/docs/v1/). This name would normally be shown to users of the system.
    name: str

    # Code given to this [Location](https://developers.intellihr.io/docs/v1/)
    code: str

    # The address of this location.
    address: str

    # Specifies whether users can select this [Location](https://developers.intellihr.io/docs/v1/) in dropdowns.
    isEnabled: bool

    representatives: LocationsDataRepresentatives

    # The custom field values for this [Location](https://developers.intellihr.io/docs/v1/)
    customFields: CustomFieldsResponse

class LocationsData(RequiredLocationsData, OptionalLocationsData):
    pass
